...
enum class BatteryChargeMode(val type: Int) {
    ...
    companion object {
        private val map = BatteryChargeMode.entries.associateBy(BatteryChargeMode::type)
...
enum class BatteryStatus(val type: Int) {
    ...
    companion object {
        private val map = BatteryStatus.entries.associateBy(BatteryStatus::type)
        fun fromInt(type: Int) = map[type]
    }
}
...
enum class BatteryHealth(val type: Int) {
    ...
    companion object {
        private val map = BatteryHealth.entries.associateBy(BatteryHealth::type)
        fun fromInt(type: Int) = map[type]
    }
}
